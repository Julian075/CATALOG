...
class CATALOG_base:
    ...
    def train(self,seed=2360402805,test=1):
        self.set_seed(seed)
                ...
                if test:
                    ...
                    with torch.no_grad():
                        ...
                        for batch_test in dataloader_test:
                            ...
                            loss_test, acc_cis_test,preds_test = projection_model(description_embeddings_test, image_features_test, text_features2,
                                                                         self.weight_Clip, target_index_test, self.t,self.sup_loss)

                            # Save predictions and targets
                            all_preds_cis.extend(preds_test.cpu().numpy())
                            all_labels_cis.extend(target_index_test.cpu().numpy())

                            running_loss_test += loss_test.item()
                            running_corrects_test += float(acc_cis_test)
                    ...
                    if self.wnb:
                        wandb.log({"loss_cis_test": epoch_loss_test, "acc_cis_test": epoch_acc_test})
                    ...
                    df_conf_matrix_cis.to_csv('conf_matrix_Base_long.csv', index=False)
    ...
    def train_OoD_Terra(self, seed=2360402805, test=1):
        self.set_seed(seed)
        unique_id = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        device = "cuda" if torch.cuda.is_available() else "cpu"
        ...
        for epoch in range(self.num_epochs):
            ...
            with torch.no_grad():
                for batch_val in dataloader_val:
                    ...
                    loss_val, acc_val, _ = projection_model(description_embeddings_val, image_features_val,
                                                            text_features, self.weight_Clip, target_index_val, self.t,
                                                            self.sup_loss)
            ...
            if epoch % print_every == 0:
                ...
                if epoch_acc_val < 1 or np.isnan(epoch_acc_val):
                    print(
                        f'💀 This model is officially trash. Accuracy: {epoch_acc_val}. Let’s not waste more compute. Training stopped.')
                    if epoch == 0:
                        test = 0
                    break
                    ...
                    with torch.no_grad():
                        ...
                        for batch_cis_test in dataloader_cis_test:
                            ...
                            loss_cis_test, acc_cis_test, preds_cis_test = projection_model(
                                description_embeddings_cis_test, image_features_cis_test, text_features2,
                        ...
                        for batch_trans_test in dataloader_trans_test:
                            ...
                            loss_trans_test, acc_trans_test, preds_trans_test = projection_model(
                                description_embeddings_trans_test, image_features_trans_test, text_features2,
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch == (self.num_epochs - 1) or counter >= self.patience:
                if test:
                    ...
                    df_conf_matrix_cis = pd.DataFrame(conf_matrix_cis)
                    df_conf_matrix_cis.to_csv('conf_matrix_Base.csv', index=False)
    ...
    def train_ID_terra(self, seed=1064200250, test=1):
        self.set_seed(seed)
        unique_id = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    def prueba_model(self,model_params_path):# to calculate the acc in test for a saved model
        ...
        with torch.no_grad():
            for batch_test in dataloader_test:
                ...
                loss_test, acc_test,preds_cis_test  = projection_model(description_embeddings_test,
                                                               image_features_test, text_features2,
                                                               self.weight_Clip, target_index_test, self.t,self.sup_loss)

                all_preds.extend(preds_cis_test.cpu().numpy())
                all_labels.extend(target_index_test.cpu().numpy())
