...
class CATALOG_base:
    ...
    def prueba_model(self,model_params_path):# to calculate the acc in test for a saved model
        ...
        dataloader_cis_test = self.dataloader(torch.load(self.path_features_D['cis_test']), self.batch_size, self.dataset)
        dataloader_trans_test = self.dataloader(torch.load(self.path_features_D['trans_test']), self.batch_size, self.dataset)
