...
class CATALOG_base:
    ...
    def train_HPO(self): # For the hyperparameters optimizations
        self.set_seed(42)
        unique_id = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        device = "cuda" if torch.cuda.is_available() else "cpu"
        ...
        for epoch in range(self.num_epochs):
            ...
            if counter >= self.patience:
                print(f'Validation acc has not improved for {self.patience} epochs. Stopping training.')
                break
        return [acc_best,model_params_path]
    def evaluate_test_HPO(self,model_params_path):# For the hyperparameters optimizations
        ...
        with torch.no_grad():
            for batch_cis_test in dataloader_cis_test:
                ...
                loss_cis_test, acc_cis_test,_  = projection_model(description_embeddings_cis_test,
                                                               image_features_cis_test, text_features2,
                                                               self.weight_Clip, target_index_cis_test, self.t)
            ...
            for batch_trans_test in dataloader_trans_test:
                ...
                loss_trans_test, acc_trans_test,_  = projection_model(description_embeddings_trans_test,
                                                                   image_features_trans_test, text_features2,
                                                                   self.weight_Clip, target_index_trans_test, self.t)
        ...
        return epoch_acc_cis_test, epoch_loss_cis_test, epoch_acc_trans_test, epoch_loss_trans_test
